...
class SimpleScanViz(Node):
    ...
    def _on_scan(self, scan: LaserScan):
        ...
        # 시각화 (클러스터만)
        self._publish_clusters(pts_map, clusters, stamp_for_markers)
    ...
    def _publish_clusters(self, pts: Sequence[Point2D], clusters: List[List[int]], stamp: BuiltinTime):
        ...
        def color_k(k: int):
            # 고정 팔레트 + 랜덤 미묘 변화
            base = [
                (1.0, 0.3, 0.3), (0.3, 1.0, 0.4), (0.3, 0.6, 1.0),
                (1.0, 0.8, 0.3), (0.7, 0.4, 1.0), (0.3, 1.0, 1.0),
            ]
            r, g, b = base[k % len(base)]
            jitter = 0.08 * (random.random() - 0.5)
        ...
        if self.show_centers:
            ...
            mc.scale.x = self.pt_scale * 2.2
            mc.scale.y = self.pt_scale * 2.2
            mc.scale.z = self.pt_scale * 2.2
            mc.color.r = 1.0; mc.color.g = 1.0; mc.color.b = 1.0; mc.color.a = 0.95

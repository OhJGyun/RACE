...
class LaneSelectorNode(Node):
    ...
    def _get_current_pose_from_tf(self) -> Optional[Tuple[float, float, float]]:
        """Get current pose from TF (map->base_link or map->ego_racecar/base_link for sim)"""
        try:
            # Try simulation frame first (ego_racecar/base_link), fallback to real car (base_link)

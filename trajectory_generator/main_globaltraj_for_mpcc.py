...
# ----------------------------------------------------------------------------------------------------------------------
# SKIP FIRST OPTIMIZATION FOR MPCC (MPCC does its own optimization)
# ----------------------------------------------------------------------------------------------------------------------
# For MPCC, we only need track boundaries (avoidance optimization)
# Skip the tight width_opt optimization

print("=" * 80)
print("MPCC MODE: SKIPPING FIRST OPTIMIZATION (width_opt)")
print("Only generating track boundaries with avoidance_opt = %.3fm" % pars["optim_opts"]["avoidance_opt"])
print("=" * 80)

# Skip first optimization - go directly to avoidance optimization

# ----------------------------------------------------------------------------------------------------------------------
# AVOIDANCE OPTIMIZATION (ONLY OPTIMIZATION FOR MPCC) -----------------------------------------------------------------
# ----------------------------------------------------------------------------------------------------------------------
# Use avoidance_opt to get track boundaries for MPCC

print("\n" + "=" * 80)
print("TRACK BOUNDARY GENERATION: avoidance_opt = %.3fm" % pars["optim_opts"]["avoidance_opt"])
print("=" * 80)

# Load original reftrack and optimize with avoidance_opt
reftrack_interp = helper_funcs_glob.src.prep_track.prep_track(reftrack_imp=reftrack_imp,
                                                               reg_smooth_opts=pars["reg_smooth_opts"],
                                                               stepsize_opts=pars["stepsize_opts"],
                                                               debug=False,
                                                               min_width=imp_opts["min_track_width"])[0]

# Get normvec and a_interp
...
# Run optimization with avoidance_opt
if opt_type == 'mincurv':
    ...
    raise ValueError('opt_type not supported for MPCC mode')

print("INFO: Track boundary optimization completed")
print("      Using avoidance_opt = %.3fm" % pars["optim_opts"]["avoidance_opt"])
...
# ----------------------------------------------------------------------------------------------------------------------
# Calculate track boundaries from the optimized raceline

print("\n" + "=" * 80)
print("CALCULATING TRACK BOUNDARIES")
print("=" * 80)

# Calculate normal vectors for the trajectory
...
veh_bound_left = trajectory_opt[:, 1:3] + normvec_normalized_opt * avoidance_opt / 2
veh_bound_right = trajectory_opt[:, 1:3] - normvec_normalized_opt * avoidance_opt / 2
...
print("\n" + "=" * 80)
print("EXPORTING MPCC FORMAT")
print("=" * 80)

# Use avoidance optimization results for MPCC
# - centerline: optimal raceline with avoidance_opt
# - left: left boundary
# - right: right boundary

# Calculate spline derivatives for centerline
centerline_xy = trajectory_opt[:, 1:3]

# Calculate heading and derivatives
centerline_cl = np.vstack((centerline_xy, centerline_xy[0]))
el_lengths_center = np.sqrt(np.sum(np.power(np.diff(centerline_cl, axis=0), 2), axis=1))

psi_center, kappa_center = tph.calc_head_curv_num.calc_head_curv_num(
    path=centerline_xy,
    el_lengths=el_lengths_center,
    is_closed=True)

# Calculate spline derivatives (dx/ds, dy/ds)
dx_ds = np.cos(psi_center)
dy_ds = np.sin(psi_center)
center_derivatives = np.column_stack((dx_ds, dy_ds))

# Calculate track widths
left_widths = np.zeros(len(centerline_xy))
right_widths = np.zeros(len(centerline_xy))

for i in range(len(centerline_xy)):
    left_widths[i] = np.linalg.norm(veh_bound_left[i] - centerline_xy[i])
    right_widths[i] = np.linalg.norm(veh_bound_right[i] - centerline_xy[i])
...
centerline_file = os.path.join(mpcc_export_dir, "centerline_waypoints.csv")
np.savetxt(centerline_file, centerline_xy, delimiter=',', fmt='%.18e')
print(f"✓ Exported: centerline_waypoints.csv ({len(centerline_xy)} points)")

# 2. center_spline_derivatives.csv
derivatives_file = os.path.join(mpcc_export_dir, "center_spline_derivatives.csv")
np.savetxt(derivatives_file, center_derivatives, delimiter=',', fmt='%.18e')
print(f"✓ Exported: center_spline_derivatives.csv")

# 3. left_waypoints.csv
left_file = os.path.join(mpcc_export_dir, "left_waypoints.csv")
np.savetxt(left_file, veh_bound_left, delimiter=',', fmt='%.18e')
print(f"✓ Exported: left_waypoints.csv")

# 4. right_waypoints.csv
right_file = os.path.join(mpcc_export_dir, "right_waypoints.csv")
np.savetxt(right_file, veh_bound_right, delimiter=',', fmt='%.18e')
print(f"✓ Exported: right_waypoints.csv")

# 5. track_widths.csv
widths_file = os.path.join(mpcc_export_dir, "track_widths.csv")
np.savetxt(widths_file, track_widths, delimiter=',', fmt='%.18e')
print(f"✓ Exported: track_widths.csv")

print(f"\nINFO: MPCC files exported to: {mpcc_export_dir}")
print(f"      Centerline: optimal raceline (avoidance_opt = {avoidance_opt:.3f}m)")
print(f"      Left/Right: boundaries at ±{avoidance_opt/2:.3f}m from centerline")
print(f"      All files: {len(centerline_xy)} waypoints")
